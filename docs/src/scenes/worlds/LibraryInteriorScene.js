...
export default class LibraryInteriorScene extends BaseWorldScene {
  constructor() {
    super(SCENE_KEYS.LIBRARY_INTERIOR);
    this.exitBounds = null;
    this.ladderClimbBounds = null;
    this.librarianWitchNpc = null;
    this.librarianWitchBubble = null;
  ...
  createWorld() {
    ...
    const interiorBackground = this.add.image(gameState.width / 2, gameState.height / 2, 'libraryInterior')
      .setOrigin(0.5)
      .setDepth(1)
      .setScrollFactor(0);
    const backgroundFrame = interiorBackground.texture.get();
    if (backgroundFrame) {
      interiorBackground.displayWidth = gameState.width;
    ...
    this.exitBounds = new Phaser.Geom.Rectangle(104, 334, 134, 226);
    this.ladderClimbBounds = new Phaser.Geom.Rectangle(ladderX - 42, upperFloorY, 84, floorY - upperFloorY);

    this.add.rectangle(exitX, exitY, this.exitBounds.width, this.exitBounds.height, 0xc49c66, 0.08)
      .setDepth(2)
      .setStrokeStyle(2, 0xf2d28c, 0.24)
      .setScrollFactor(0);
    this.add.text(exitX, 316, 'Exit Library', {
      fontFamily: 'Georgia',
      fontSize: '18px',
      color: '#f8e7bf',
      fontStyle: 'bold',
    ...
    this.createLibrarianWitchNpc((exitX + ladderX) / 2, floorY);
    ...
    this.createCollisionZone(gameState.oneWayPlatforms, 0, upperFloorY, 930, 24);
    this.createCollisionZone(gameState.oneWayPlatforms, 1024, upperFloorY, 112, 24);
    this.createExitDialog();
  }

  createLibrarianWitchNpc(x, footY) {
    if (!this.textures.exists('librarianWitchNpc')) {
      return;
    }

    const footOriginY = 159 / 192;
    this.librarianWitchNpc = this.add.sprite(x, footY, 'librarianWitchNpc', 0)
    ...
    if (this.anims.exists('npc.librarianWitch.idle')) {
      this.librarianWitchNpc.play('npc.librarianWitch.idle');
    }

    const visibleHeight = (159 - 28) * this.librarianWitchNpc.scaleY;
    this.createLibrarianWitchBubble(x, footY - visibleHeight - 50);
    this.librarianWitchNpc.on('pointerdown', () => this.openLibrarianWitchBubble());
  }

  createLibrarianWitchBubble(x, y) {
    ...
    const message = this.add.text(x, bubbleY - 30, 'Can I help you with books you need?', {
      fontFamily: 'Georgia',
      fontSize: '18px',
      color: '#2e2040',
      fontStyle: 'bold',
    ...
    this.librarianWitchBubble = {
      visible: false,
      elements: [panel, message, buyButton, sellButton, buyLabel, sellLabel],
    };
  }

  openLibrarianWitchBubble() {
    if (!this.librarianWitchBubble || this.librarianWitchBubble.visible) {
      return;
    }

    this.librarianWitchBubble.visible = true;
    this.librarianWitchBubble.elements.forEach((element) => element.setVisible(true));
  }

  createCamera() {
    super.createCamera();
    this.cameras.main.setZoom(1);
    this.cameras.main.setDeadzone(180, 150);
  ...
  createExitDialog() {
    ...
    const panel = this.add.rectangle(panelX, panelY, panelWidth, panelHeight, 0x120f14, 0.97)
      ...
      .setDepth(depth + 1)
      .setVisible(false);
    const title = this.add.text(panelX + (panelWidth / 2), panelY + 44, 'Exit Library?', {
      fontFamily: 'Arial',
      fontSize: '24px',
      color: '#faefd6',
      fontStyle: 'bold',
    ...
    yesButton.on('pointerdown', () => this.confirmExitLibrary());
